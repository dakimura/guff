...
Action は guff の解析キャッシュを実行間で保持するため、プルリクエストでは変更
された分だけを再 lint します。GitHub ホストランナー上の 113 パッケージのモ
ジュールでの実測は、cold 7.9秒・無変更 0.2秒・依存元の多いファイルを1つ変更
して 4.2秒。マトリクス構成、キャッシュ容量の調整、セルフホストランナーについ
ては [`docs/CI.md`](docs/CI.md) を参照してください。
