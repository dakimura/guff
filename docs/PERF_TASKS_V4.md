...
> `golist/` ディレクトリを誰も書かなくなったためです。
> **→ 2026-08-15 に修理（V5 §2.2）。実はバグは 2 つあり、
> C-7 は「2026-07-31 以降」ではなく *書かれた日から一度も* HIT していませんでした。
> 効果も −15% ではなく −0.04s / −2.3% です。**
...
> **2026-08-15 訂正（V5 §2.2）: 修理済み。ただしこの節の診断は半分でした。**
> - peek 先が無い件（下記）は**1 つ目**のバグ。直すと start はしますが**必ず MISS します**。
> - **2 つ目**は「peek したグラフを `refine` に通していない」こと。生のドライバ応答は
>   1792 pkgs / 293 roots、実際に解析されるのは 1616 / 118 で、**指紋が合うはずがありません**。
>   `GUFF_NATIVE_LIST=off` で C-7 が書かれた当時の経路に戻しても MISS するので、
>   **C-7 は書かれた日から一度も HIT していません**でした。
> - 下の「直したときの見込み」は `--no-cache` 条件の数字です。**実測は −0.04s / −2.3%**。

**この節を書いた時点では未修理でした。性能タスクではなく、落ちている機能の修理です。**
...
> **2026-08-15 追記: この節の候補 1 と 3 は第5弾で決着しました。
> 着手する前に [`docs/PERF_TASKS_V5.md`](PERF_TASKS_V5.md) を読んでください。**
> - **候補 1（コメント再パースの共有）は NO-GO**。「thread-local なら RSS 増はほぼゼロのはず」は
>   **外れ**でした。ランナーの wavefront はパッケージの analyzer を別々のワーカーに散らすので
>   thread-local は一度も当たらず（CPU +1.0%）、当たる形（プロセス共有）にすると
>   **RSS +0.37 GiB** を払います。V5 §3.2 にトレードオフ曲線があります。
> - **候補 3（C-7）は修理済み**。ただし **−0.29s / −15% は出ません**（V5 §2.2）。
>   下の §5 の見積りは `--no-cache` 条件のもので、実測は **−0.04s / −2.3%** です。
> - 候補 2 / 4 は未着手のままです。
...
1. ~~**コメント再パースの「共有」を、RSS を増やさない形で。**~~ → **NO-GO（V5 §3.2）。** V4-1 は 1 回のパースを
...
3. ~~**C-7 の修理**（§5）。永続キャッシュ 2 回目で上限 −0.29s / −15%。要ユーザー判断。~~
   → **第5弾で修理済み。実測 −0.04s / −2.3%**（V5 §2.2）。**バグは 2 つあり**、
   §5 が書いた「peek 先が無い」は 1 つ目でした。2 つ目は「peek したグラフを
   `refine` に通していない」で、そのせいで **C-7 は書かれた日から一度も HIT していません**でした。
...
- **コメント再パースを共有しない。** V5 §3.2 でトレードオフ曲線まで実測済み。
