...
| goprintffuncname | ✅ | |
| gosec | 🟡 | 主要ルール対応（G115 = 整数オーバーフロー変換はレンジ解析ごと、G101 = zxcvbn のエントロピー推定ごと移植済み）。G113 / G116–G121 / G123 / G201 / G304–G305 / G307 / G601 と G7xx の完全な taint エンジンは DEFERRED |
| gosmopolitan | ✅ | |
...
| unparam | ✅ | |
| unqueryvet | ✅ | |
| unused | 🟡 | 単一パッケージ（型・定数・メソッド・const グループ）。到達可能性は honnef と同じく根からのグラフ彩色。whole-program 版と `//go:linkname` / `//go:cgo_export_*` の根は DEFERRED |
| usestdlibvars | ✅ | |
| usetesting | ✅ | |
| varnamelen | ✅ | |
