...
> ### ⚠️ 第4弾があります → [`docs/PERF_TASKS_V4.md`](./PERF_TASKS_V4.md)
>
> **新しく着手するなら V4 §6「次にやる人へ」から選んでください。** V4 は下の §4.5 の
> 候補を**測って 3 つ消し込みました**（B-9 / O(候補×AST) / アロケータ）。とくに:
>
> - **§4.5 候補 6 の grep（`for file in pass.files()`）は役に立ちません。** V4 §4.2。
>   正常な 1 ファイル 1 周の walk が数百件ヒットして入れ子と区別できません。
>   プロファイルの `--callers` で呼び元 CPU を見てください。
> - **§4.5 候補 3（`typecheck_roots` の再評価 / B-9）は最終 NO-GO。** V4 §4.1 に
>   スレッド数を振った実測（4→10 で wall −0.04s / CPU +1.2s）があります。
> - **§4.5 候補 5（アロケータ / `Expr::clone`）も NO-GO。** V4 §4.3。
> - **下の「regress full が FAIL のまま」は解消しました。** V4 §7.2 で **PASS**
>   （wall 2.440s < limit 2.510s）。`--update-baseline` は不要です。
