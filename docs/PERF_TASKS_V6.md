...
> - **§3.4 コメント再パースの共有は、やはりやらない。** ただし理由が第5弾と違います。
>   RSS ではなく**上限そのものが 0.04s wall しかない**（V4-1 が 1 回あたりを既に安くしたため）。
> - **§3.5 native lister の BFS バリアは撤廃しない。** level が広い（最大 721）ので
...
> **その引き直しは今回やりました（§3.4）。結論は変わらず NO-GO ですが、理由が変わります。**
...
### 3.4 コメント再パースの共有 — **上限を測って NO-GO（0.04s wall）**

第5弾 §3.2 は「共有すると効くが RSS を +0.37 GiB 払う」で NO-GO でした。
**§2.1 でパッケージの寿命が縮んだので、その RSS の前提は崩れています。**
そこで**実装する前に上限だけ測りました**（`PERF_TASKS_V2.md` ルール14）。

V6 後のプロファイルで、コメントを再パースする 3 者の inclusive CPU は:

| CPU | 呼び元 |
|---:|---|
| 0.334s | `gocritic::run_comment_checks` |
| 0.243s | `godot::run`（うち `reparse_with_comments` 0.216s） |
| 0.122s | `revive::util::reparse_with_comments` |
| **0.70s** | **合計**（うちパース本体は ~0.55s。残りは検査そのもの） |

3 回を 1 回にできたとして **削れるのは ~0.37s CPU**、analyze の並列度 8.8× で
**wall にすると −0.04s** です。**ルール14 の 0.1s に届きません。**

> **V4 §6 候補 1 の「上限 ~0.8s CPU ≒ analyze wall −0.09s」は古い数字です。**
> あれは **V4-1（`PARSE_COMMENTS` → `COMMENTS_ONLY`）より前**の測定で、
> V4-1 自身が 1 回あたりのパースを半分にしました。**回数を減らす余地は、
> 1 回を安くした後には、もうほとんど残っていません。**

実装コスト側も見ておくと、`reparse_with_comments` の戻り値は
`(Arc<FileSet>, File)` の**所有値**で、呼び出し元が 27 箇所あります。共有するには
`Arc<Reparsed>` へ変える必要があり、**`--watch`（長寿命プロセス）でのキャッシュ無効化**も
自前で用意することになります。**0.04s のために触る場所ではありません。**

### 3.5 native lister の BFS バリアを撤廃する — **NO-GO（バリアが遊んでいない）**
...
### 4.1 いま RSS を攻めるなら typecheck（analyze ではない）
...
### 4.2 analyze に残っている山（プロファイルは取り直してあります）
...
`object_call_name` は V5 §3.1 で memo 化が NO-GO です（現在 0.182s）。**キャッシュ局所性が変わったので
再測定する価値はありますが、同じ結論が出たらそれで 2 回目です。二度と触らないこと。**

### 4.3 `perf-ab.sh` の順番入れ替え（**この回で入れました**）

§1 のとおり、A→B 固定だと**熱ドリフトが常に B に課金されます**。
`scripts/perf-ab.sh` は**既定で 1 ラウンドおきに順番を入れ替える**ようにしました
（オプションではありません。忘れられる余地を残さないため）。
入れ替えたラウンドは出力に `(B ran first)` と出ます。

### 4.4 やってはいけない（第6弾で追加）

- **初期 ready を LPT で並べ替えない。** §3.2。
- **コメント再パースを共有しない。** §3.4。**第5弾とは別の理由**（上限が 0.04s wall）で、
  こちらは実装前に上限を測ってあります。
- **seed プールを広げない（4 のまま）。** §3.1（6 でも CPU +9%）/ §3.3（8 は 6 と同着）。
- **native lister の BFS バリアを外さない。** §3.5。計測行が既にあります。
- （第5弾から継続）`code::object_call_name` を memo 化しない（ただし §4.2 の注記あり）/
  C-7 を性能改善として見積もらない。
