...
> ---
>
> ### 📌 これから新しく高速化タスクに着手する人へ
>
> **このファイルの Task 1〜5 はすべて完了（DONE）しています。**
> 新しく着手するタスクは **[`docs/PERF_TASKS_V2.md`](./PERF_TASKS_V2.md)（第2弾・全33本）** から
> 選んでください。
>
> ただし、**このファイルの §0（絶対ルール）・§1（計測ハーネス）・§2（findings 同一性の検証）・
> §3（本番ゲート）は第2弾でもそのまま有効**です。第2弾はそれらを再掲せず参照しているので、
> **先にこのファイルを最後まで読んでから** V2 に進んでください。
...
**parse の残りを詰めるべきか（2026-07-26 当時の結論: 詰めない → 2026-07-31 再訪で GO）:**
依存クロージャ 149.9MB / 12318 ファイルをシングルスレッドで実測すると、フルパース 4.08s →
本体スキップ 2.04s、うち**スキャン 0.87s（43%）/ AST 構築 1.17s（57%）**（22.3M トークン）。
当時は「生バイト読み飛ばしでも wall 換算 0.1s 未満」と見積もって打ち止めにしたが、
C-3c 後に seed が critical path の本丸になったため **2026-07-31 に再計測して着手**
（`Scanner::skip_to_closing_brace`。`-j 1` seed **−0.21s**。詳細は `PERF_TASKS_V2.md` 冒頭）。
