...
> ### 📌 セッションを引き継いだ人はここから
>
> **完了済み: S-1 / P0-1 / P0-2 / A-5 / B-0 / B-8。** 各タスク節末尾の `### DONE` に実測値があります。
>
> **性能タスクの前に、まず [§8「次セッションへの引き継ぎ」](#8-次セッションへの引き継ぎ--性能タスク中に見つかった別問題2026-07-27)
> を読むこと。** 性能作業中に見つけた**性能以外の問題が 3 件**（正しさのバグ 1・テストの穴 1・計測作法 1）
> 未修理で残っています。とくに:
>
> - **X-2 を知らないと事故ります**: `cargo test --workspace` は**いま `main` で 11 本落ちています**
>   （`guff-govet` の `checks_test`）。原因は特定済みで**あなたの変更のせいではありません**。
>   §4 のテンプレートの「`cargo test --workspace` が通る」は、
>   **この既知の 11 本以外が通ること**と読み替えてください。
...
> §0-9（1コミット1論点）に従い**あえて直さずに残してあります**。
> 性能タスクと混ぜて直さないこと。着手する順は **X-1 → X-2** を推奨します
> （X-1 は正しさのバグ、X-2 はそれを守るテストの穴）。
