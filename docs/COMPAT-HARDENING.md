...
### 2026-08-25（続き 44）— revive の `--fix` は**上流がほぼ全リポジトリで壊れている**。台帳に「進んでいる」用の枠を 1 つ足した

pending 45 件のうち **9 件が `revive-*`** ——
1 つの linter が 9 通りの config で並んでいるだけなので、
機構を 1 本入れれば 9 件同時に閉じる。そこから始めた。

#### 機構は per-rule ではない

revive の fix は `lint.Failure.ReplacementLine`（**行まるごとの置換テキスト**）で、
golangci はそれを `LineStart(開始行)`〜`EndOfLinePos(終了行)` の
**1 つの edit** に変換する。AST の span ではなく**行**なのがポイントで、
rule 側は**ソース行に正規表現を当てて組み直す**
（`errorf`: `m[1] + prefix + ".Errorf(" + m[2] + ")" + m[3]`）。

`Failure` に `replacement_line` を足し、`errorf` で埋め、
report 時に行 edit を作る。上流が `ReplacementLine` を設定する rule は
`errorf` と `range` の 2 つだけなので表面積は小さい。

#### `EndOfLinePos` は -1 しているのに、改行を食う

素直に移植したら**空行が 1 行増えた**。
上流は `LineStart(line+1) - 1` ＝**改行の手前**を指すので、
`ReplacementLine + "\n"` を入れると元の改行が残るはずで、実際そうなった。

上流でそうならないのは `fixer.go:104` が
`token.Pos` を **`int` にキャストしてそのままバイトオフセットとして使う**から
（`Fset.Position` を通さない）。適用される値が 1 つずれて改行を含む。
guff のオフセットは本物なので、**意図のほうを直接書く**しかない。

#### そして「上流が何も書かない」ケースが出た

`revive` ケース（27 ファイル）だけ、
**golangci-lint は 1 バイトも書かないのに guff は書く**。
ハーネスが拒否した:

```
revive: REFUSING to hold — golangci-lint --fix changes nothing here
        and guff writes 11 diff line(s). Fix guff.
```

続き 38 の omitzero と同じ形なので、`pending` は**この向きを絶対に保持しない**。
自分で入れたルールに自分で止められた形で、それは正しい。

#### 上流の挙動は**事故**で、再現できない

`pkg/golinters/revive/revive.go` は
`Fset.File(token.Pos(failure.Position.Start.Offset))` ——
**バイトオフセットを FileSet 全体の位置として**渡してファイルを引き、
「引けたファイル名 ≠ failure のファイル名」なら fix を捨てる。
コメントには「cgo を飛ばすため」と書いてあるが、実際にやっているのは
**FileSet の先頭にあるファイル以外の fix を全部捨てる**こと。

推測ではなく測った（golangci-lint 2.12.2）:

| モジュールの形 | 結果 |
|---|---|
| 1 パッケージ 1 ファイル | 修正する |
| 1 パッケージ `a.go`+`b.go` | **a.go だけ修正、b.go は放置** |
| 2 パッケージ | **何も修正しない** |

同じ計算を guff の FileSet でやったら
**動いていた 9 件まで抑止された**（156→148）。
guff は依存ファイルを先に FileSet に入れるので落ちる場所が違う。
**どちらのツールも保証していない並び順に依存する挙動は、移植できない。**

#### `divergent/` —— 拒否の唯一の出口

`pending` の拒否は一般には緩めない。緩めたら omitzero が
もう一度静かに通る道を戻すことになる。代わりに**狭い専用枠**を作った。

`compat/fix/divergent/<case>.diff`:

- **手書き専用**。`regen.sh` は作らない（recorder の拒否はそのまま）。
  人が文章で「上流が間違っている」と書いた場合だけ通る。
- **`# why:` 必須**。無ければ落ちる ——
  **理由の書かれていない意図的乖離は、行儀のよい allowlist でしかない。**
- **guff のバイトが動いたら落ちる**。1 回の判断の記録であって、
  何を書いてもよい許可証ではない。
- **上流が書き始めたら落ちる**。理由が常に
  「上流は何も書かない、そしてそれは誤りだ」の形なので、
  前提が消えたら判断し直す。
- 理由は毎回ケース名の横に出る。

単体テストで 3 つの否定パスを固めた（`compat/tests/test_fixdiff.py`、19 本）。
**4 つ目（上流が書き始めたケース）はテストが実装漏れを捕まえた** ——
ルーティングが「上流が何も書かないとき」に限定されていて、
書き始めたときは普通の不一致として出てしまい、
「記録が古い」ことが伝わらなかった。

#### QF1004（続き 42）との違い

どちらも「guff のほうが正しい」だが、**向きが逆**:

| | 上流に合わせると | 判断 |
|---|---|---|
| QF1004 | **コンパイルできないコードを書く** | guff を正しいまま（実害があるので） |
| revive | **何も書かない** | guff を正しいまま（失うのは改善機会だけ） |

QF1004 は pending に入れられた（上流が書くので）。
revive は入れられない（上流が書かないので）。
**同じ「guff が進んでいる」でも、台帳に入るかどうかは
上流が何か書いたかで決まっていた** —— それが枠を足した直接の理由。

#### 測った

| | 続き 43 後 | 今回 |
|---|---:|---:|
| `--fix` が上流とバイト一致 | 148 | **156** |
| pending | 45 | 36 |
| 意図的乖離 | 0 | **1** |
| ビルドが通らない木 | 6 | 6（全部上流一致のまま） |

golden **193/193**。

#### 次にやること

1. pending 36 件。`staticcheck-s` 271 行が最大、次いで `goheader` 110、
   `testifylint-mock` 97、`govet` 88。
...
### 2026-08-26（続き 47）— `stringintconv` と `composites`。**ガードは飾りではない**

続き 46 の続き。`govet` ケースは 88 対 35 だった。

#### `stringintconv` は代替 fix を 2 つ出す。**衝突しない**

上流は `fmt.Sprint(x)` と `string(rune(x))` の 2 つを出す。
続き 38 の `omitzero` と同じ「1 診断に 2 fix」だが、**あちらと違って span が重ならない**:
Fix 1 は変換の型名（`call.Fun`）、Fix 2 は引数への挿入。

golangci の fixer は **1 つの issue の全 SuggestedFix から edit を集める**
（続き 37 で移植済み）ので、両方が適用されて
`fmt.Sprint(rune(65))` になる —— expected の出力そのもの。
**「2 つ出す」が正解で、片方に絞るのは間違い。**

#### ガード 3 つは全部「上流より少なく書く」側に倒した

| ガード | 上流 | guff | なぜ |
|---|---|---|---|
| メソッド | `NewMethodSet(V0).Len() == 0` | 宣言メソッド数 > 0 で拒否 | guff にメソッドセット計算が無い。ポインタレシーバのみの型で上流は出し guff は出さない |
| 型パラメータ | 型集合が 1 項なら可 | 型パラメータなら拒否 | 単項の型パラメータで上流は出す |
| 変換先 | `Identical(T0, string)` | `Basic(String)` そのもの | 名前付き string 型は変換を残す（上流と同じ 2 分岐） |

メソッドのガードだけは「安全側」以上の意味がある。
`String()` を持つ型を `fmt.Sprint` に書き換えると**出力が変わる**。
**少なく書くのは pending が保持できる。挙動を変えるのは保持できない。**

#### `composites` が慎重な理由は 2 つとも実務的

- **要素数がフィールド数と一致するときだけ**
  —— 短いリテラルは末尾の値に対応するフィールドが無く、名前の付けようがない
- **非公開フィールドに当たったら中止**
  —— ここから見えない名前はここに書けない

どちらも「安全のため」ではなく「**書けないから書かない**」。
...
| `--fix` が上流とバイト一致 | 160 | 160 |
| pending | 32 | 32 |
| `govet` ケースが書く行数 | 35 / 88 | **55 / 88** |

golden **193/193**。

`govet` の残りは `hostport`（分岐が複数＋`net` の import）と
`inline` / `inline_local`（gopls のインライナ本体）。
**このケースが 88/88 になるのは当面先**だが、
analyzer 4 つの fix は今日から利用者に届く。

#### 次にやること

1. `hostport`。`fmt.Sprintf("%s:%d", host, port)` →
   `net.JoinHostPort(...)`。リテラル port / 非リテラル port の 2 形。
2. pending 32 件。`staticcheck-s` 271 行が最大。
3. `inline` / `inline_local` は別規模。当面手を付けない。

---

### 2026-08-26（続き 48）— `hostport`。**「直したくなるが直してはいけない」が 2 つ**

続き 47 の続き。`govet` ケースは 88 対 55 だった。

`fmt.Sprintf("%s:%d", host, port)` → `net.JoinHostPort(host, fmt.Sprintf("%d", port))`。
`"%s:%s"` のほうは port が既に文字列なので `net.JoinHostPort(host, port)`。

#### `net` の import を足さない

`AddImport` は続き 39 で移植済みなので通したくなる。**通してはいけない。**
この fix は `net.Dial` に渡る呼び出しにしか発火しないので、
ファイルには必ず `net` がある。足せば上流が書かない import を書くことになる。

#### 名前付き定数を `"123"` に畳まない

上流は**整数リテラルだけ**をその場で 10 進文字列にし（`0x7B` → `"123"`）、
それ以外は名前付き定数であっても `fmt.Sprintf("%d", …)` で包む。

これは慎重さではなく**意味の保存**で、上流のコメントがそう書いている:
`k` を `"123"` に畳むと **`k` という名前への参照が消える**。
`strconv.ParseInt(lit, 0, 64)` の基数推定
（`0x`/`0b`/`0o`・先頭 `0`・`_` 区切り）も一緒に移植した。

#### edit は粒度を保つ

上流は「Use granular edits to preserve original formatting」と書いて
呼び出し全体を置換しない。引数の元の書き方が残る。
...
| `--fix` が上流とバイト一致 | 160 | 160 |
| pending | 32 | 32 |
| `govet` ケースが書く行数 | 55 / 88 | **70 / 88** |

golden **193/193**。

#### この一連（続き 45–48）で効いたこと

govet の fix を 7 つ移植して、**上流のガードを読み飛ばさない**のが一番効いた:

| ガード | 素朴に見える | 実際の理由 |
|---|---|---|
| `stringintconv` のメソッド判定 | 保守的 | `String()` があると**出力が変わる** |
| `composites` の非公開フィールド中止 | 保守的 | **ここから見えない名前は書けない** |
| `composites` の要素数一致 | 保守的 | 短いリテラルは**対応するフィールドが無い** |
| `hostport` の定数を畳まない | 保守的 | **名前への参照が消える** |
| `assign` の全ペア限定 | 保守的 | 部分削除は**カンマの処理**が要る |

**どれも「安全のため」ではなく「そうしないと壊れるから」だった。**
ガードを緩めるのは、たいてい理由を読んでいないということ。

#### 次にやること

1. `inline` / `inline_local`。呼び出しを関数本体で置き換える gopls のインライナ。
   これまで移植した「span を差し替える」種類とは規模が違う。
   **`govet` ケースが 88/88 になるのはこれ次第。**
2. pending 32 件。`staticcheck-s` 271 行が最大。

---

### 2026-08-26（続き 49）— `godot`。**フィクスチャが通ることと、正しいことは別**

pending の 0 バイト組で 2 番目に大きい 59 行。1 回でバイト一致した。

#### 仕組み

godot は `Issue.Replacement` に**行まるごと**（`.` を挿し込んだ raw 行）を入れ、
golangci は「issue 位置 〜 行末」をそれで置換する。
revive の `ReplacementLine`（続き 44）と同じ「行が単位」の形。

**ただし改行の扱いが逆。** golangci は revive では `Replacement + "\n"` を書くので
end が改行を含む必要があったが、godot では `Replacement` をそのまま書くので
end は**改行の手前**で止める。続き 44 で「上流の書いてあることではなく
起きることに合わせる」と書いたが、両者を並べると辻褄が合っていた ——
**`"\n"` を足すほうだけが改行を食う。**

#### 近似で通ってしまうところを、通さなかった

フィクスチャは `//` 行コメントだけなので、
**「trim した行末に `.` を足す」だけでバイト一致する**。
それでも godot 本来の写像を実装した:
コメント本文を raw 行の中から探し（`strings.Index`）、その trim 済み末尾に挿す。
...
コーパスに `/* */` が無いので tier は緑になる。**それは根拠にならない。**
続き 44 で revive の「コーパスに合わせて緑にする」案を退けたのと同じ形で、
今回は**自分が緑を取れる近似を持っていた**ぶん誘惑が強かった。

除外パターンで `<godotSpecialReplacer>` に置き換わった行は
raw 行から探せないので、その場合は fix を出さない（少なく書く）。

#### 測った

| | 続き 48 後 | 今回 |
|---|---:|---:|
| `--fix` が上流とバイト一致 | 160 | **161** |
| pending | 32 | **31** |
...
1. pending 31 件。0 バイト組の残りは `staticcheck-s` 271 /
   `goheader` 110 / `testifylint-mock` 97 / `gocritic` 76 / `protogetter` 42 /
...
### 2026-08-26（続き 52）— staticcheck の S チェック 3 つ。**レンダラを 2 つ持っている意味**

pending 最大の `staticcheck-s`（271 行）へ。22 チェックに fix がある。

#### pin 版の確認が早速効いた

続き 51 で記録した手順どおり最初に確認したところ:

- pin は `honnef.co/go/tools v0.7.0`
- **module cache に v0.7.0 は無い**（v0.3.2 / v0.5.1 / v0.6.1 だけ）
- ローカル checkout は `v0.7.0` タグを持ち、**HEAD はそこから 69 コミット先**

`git show v0.7.0:simple/s1002/s1002.go` でタグの内容だけを読んだ
（作業ツリーには触れない）。
...
| check | 上流の edit |
|---|---|
| S1012 | `code.EditMatch` —— マッチ全体を置換パターンの**再印字**に差し替え |
| S1002 | `edit.ReplaceWithString(expr, r)` —— `r` は**メッセージが引用している文字列そのもの** |
| S1004 | `edit.ReplaceWithPattern` —— `==` / `!=` で別パターン |

S1002 は「答えがメッセージに書いてある」形だった。
guff も同じ文字列を計算済みだったので、**スパンを足すだけ**で済んだ。

#### `render_expr` と `render_node` を取り違えた

S1012 で最初 `render_expr` を使った。これは**診断メッセージ用の手書きレンダラ**で、
上流の fix 経路は `format.Node`（go/printer）。

二項演算子の空白などで食い違う。
メッセージなら見えないが、**`--fix` はディスクに書くバイト**。
`render_node`（go/printer 経由）に差し替えた。

続き 35 の「`expr_string` は重複ではない、7 つあり上流は 4 系統」と同じ話が、
**今度は書き込み側で**出た形。
**レンダラが 2 つあるのは、答えるべき質問が 2 つあるから。**

#### 3 チェックが 4 ケース目を閉じた

`staticcheck-checks-glob` が上流と完全一致になった。
このケースは `checks` に絞った集合を有効にしていて、
たまたまこの 3 つで足りていた。
**チェック単位の作業が、ケース単位で不連続に効く。**

#### 測った

| | 続き 51 後 | 今回 |
|---|---:|---:|
| `--fix` が上流とバイト一致 | 162 | **163** |
| pending | 30 | **29** |
| `staticcheck-s` が書く行数 | 0 / 271 | **32 / 271** |

golden **193/193**。
`staticcheck-checks-all` / `-default` も 27 → 36 行に動いたので再記録した。

#### 次にやること

1. `staticcheck-s` の残り 19 チェック。
...
### 2026-08-27（続き 61）— `dupword`。**ピン版を取りに行った回**、そして近似を書かないために止めた回

続き 60 の続き。pending 25 件、`dupword` は 36 行中 **0 行**。

#### 展開済みは v0.1.8、ピンは v0.1.7

`~/Library/Caches/go/pkg/mod/cache/` にあったのは **v0.1.8**、
golangci-lint 2.12.2 の `go.mod` が指すのは **v0.1.7**。
`go mod download github.com/Abirdcfly/dupword@v0.1.7` で取ってきて比べたら、
`Check` のループが**実際に違った**:
...
続き 47 のメモが繰り返し言っていること
（**checkout はピン版ではない**）が、また当たった。

#### 上流のバグをそのまま書く

`"a a b"` に対して上流が書くのは **`"a "`**。`"a b"` ではない。

理由は `else if` の連鎖:
最後の文字 `b` が**新しい単語を始める**ので、
最初の分岐（word start）が反復を取ってしまい、
`i == len(raw)-1` の分岐に**到達しない**。
`b` はどこにも書かれない。

もう 1 つ: `excludeWords` に弾かれた重複語は
どちらの分岐にも入らないので、
**報告されないまま書き換えからは消える**。

バイト一致とは、この 2 つを再現すること。

#### 文字列リテラル側は**あえて止めた**

上流は `strconv.Unquote` → 書き換え → `strconv.Quote` で戻す。
guff の Go 厳密な `quote`/`is_print`（Go 生成の print テーブル付き）は
`guff-staticcheck` の `gostd` にあり、`guff-comment` からは層が違う。

近似の quoter を書けば残り 3 行は埋まる。書かなかった:
**それは他人のソースファイルに近似を書き込むこと**で、
このドキュメントが「記録された diff から形を推測するな」と
言い続けているのと同じ話。

`gostd::strconv` を共有の場所へ移すのは別 PR。

#### 測った

| | 続き 60 後 | 今回 |
|---|---:|---:|
| `--fix` が上流とバイト一致 | 167 | 167 |
| pending | 25 | 25 |
| `dupword` | 0 / 36 行 | **33 / 36 行** |

コメント側は**全行が上流と一致**。残り 3 行は文字列リテラルだけ。

golden **193/193**、269 スイート / **3,274** テスト。

#### 次にやること

1. `gostd::strconv` を共有クレートへ移して `dupword` を閉じる。
2. pending 25 件。`protogetter` 42 / `nlreturn` 34 は
...
### 2026-08-27（続き 65）— fixer にパス境界。**下流では捕まえられない事故だった**

続き 64 で GOROOT の 1 ファイルを壊した。復元はしたが、
**同じことがまた起こせる状態**だったので閉じる。

#### 上流にはこの問いが立たない

golangci の fixer は edit を **issue のファイルパス**でまとめる:

```go
editsByLinter[issue.FilePath()][issue.FromLinter] = append(...)
```

オフセットも `goanalysis` が `diag.File` に対して計算するので、
**edit が別のファイルを指す経路が無い**。

guff は edit を `FileSet` 経由で解決する。
その `FileSet` には**実行中にパースした全ファイル**が入っていて、
GOROOT も含む。別の `FileSet` で作られた `Pos` もただの数値なので、
`fset.file()` は**その範囲を占める任意のファイル**を返す。

#### 入れた規則

**edit は、その issue が指すファイルに落ちなければならない。**
落ちなければ捨てる。
...
`same_file` は `./` の有無と相対・絶対の違いを吸収する ——
issue の filename と `FileSet` が返す filename は
実行の別々の場所で作られるので**形は違いうる**。
違ってはいけないのは**どのファイルか**のほう。

#### なぜ下流で捕まえられなかったか

事故の bytes は、

- オフセットが**範囲内**で（`edit.end > content.len()` を通り抜ける）、
- ファイルが**書き込み可能**で、
- **間違ったファイルに対する妥当な edit**だった。

`--fix` の出口には、これを疑う材料が何も無い。
だから**入口で**、edit と issue の対応を要求する。
...
**193 ケースで数字が 1 つも動かない**ことが、この防護の検証になっている ——
正当な edit を 1 つも落としていない。

unit test を 1 本追加。事故を縮めた形（issue は `offender.go` について、
edit は `victim.go` を指す）で、**ガードを外すと実際に victim を壊す**ことを
確認済み。269 スイート / **3,275** テスト。

#### この 3 件は同じ根から出ている

| | 症状 |
|---|---|
| 続き 55 | goheader の `comment_line` を Pos で取ろうとした（行番号にして回避） |
| 続き 63 | nolintlint の Pos が別 FileSet（設計判断として保留） |
| 続き 64 | その保留した変更を**入れたまま測定し**、GOROOT を壊した |
...
### 2026-08-27（続き 73）— `staticcheck-qf` の「差 4 行」は QF1005 の欠陥 7 個だった。うち 1 つは**呼び出しを消していた**

続き 72 の続き。pending 16 件（`parens` が divergent へ移って 17）。
`staticcheck-qf` は 395 行対 395 行で、**上流と同じ行を消して違う 4 行を書く**
——「ほぼ閉じている」と読める形。

#### 先に探索 fixture を書いた

続き 69 の教訓（fixture が 1 形しか通さないと他の枝が見えない）を
**golden を触る前に**適用した。scratchpad に 24 形の `.go` を並べ、
同じ config で `golangci-lint --fix` と `guff --fix` を別コピーに当てて `diff`。

**上流と違う行が 7 つ出た。** fixture が持っていたのは
`math.Pow(x, 0..3)` と `math.Pow(2, 2)` / `(2, 3)` の 6 形だけだった。

#### 一番重い 1 つ: `--fix` が関数呼び出しを消していた

```go
_ = math.Pow(g(), 0)   // 上流: 触らない / guff: _ = 1.0
```

上流は `MayHaveSideEffects` を **`n` を読むより前**で `continue` している。
guff は `n >= 2 && may_have_side_effects(x)` で見ていた ——
「底が繰り返されるのは n>=2 のときだけ」という理屈は通っているが、
**n==0 の置換は底を含まない**ので、`g()` ごと消える。

そしてこれは fix だけの話ではない。**上流はこの 4 件を報告すらしない**
（`continue` が report より前）。golden が 193/193 だったのは
fixture に側効果のある底が 1 つも無かったからで、
`ok.go` に 6 形足したら **guff だけが 4 件報告している**ことが出た。
続き 69 と同じ形だが、**golden の側に出た**のは今回が初めて。

#### `needs_float64_wrap` は一度も発火していなかった

上流は `math.Pow(2, 2)` を **`float64(2 * 2)`** に書き換える。guff は `2 * 2`。

guff の述語は「記録された型は float64 に代入可能か」を訊いていた。
**`math.Pow` の引数は必ず float64 に変換済みで記録される**ので、答えは常に yes、
つまり**この関数は書かれてから一度も true を返していない**。

上流が `types.CheckExpr` で**式を単独で型検査し直している**理由がこれで、
定数値のほうも当てにならない（`representable` が `constant.ToFloat` を通して
書き戻すので、`2` も `2.0` も Float 値・float64 型で記録される）。
移植は構文から untyped kind を計算し直す:
リテラルはトークン、名前付き定数は宣言型、**シフトは左オペランドだけ**、
それ以外は両辺の広いほう。

| 底 | 単独の kind | 包む |
|---|---|---|
| `2` / `1+1` / `4/2` / `1<<3` / `-2` / `(2)` | untyped int | ✔ |
| `'a'` | untyped rune | ✔ |
| `2.0` / `-2.5` / `5.0/2` / `2.0*2` / `math.Pi` | untyped float | — |
| `typedFloat`（`const float64`） | float64 | — |
| `x`（変数） | 定数ではない | — |

包むのは**積の外側 1 回**（`float64(x * x)`、`float64(x) * float64(x)` ではない）。
`n == 1` にも掛かり（`float64(2)`）、`n == 0` には掛からない（`1.0` は既に untyped float）。

#### 括弧は go/printer のもの ——**ただし `SimplifyParentheses` は名前に反して回転もする**

guff は BinaryExpr と UnaryExpr を無条件に括っていた。上流は AST を組んで
`format.Node` で印字するだけなので、括弧は printer の規則で決まる:

- 単項・一次式は**括らない** → `-x * -x`
- `*` より低い優先順位は両側で括る → `(x + 1) * (x + 1)`
- **同じ**優先順位は右だけ括る（`*` は左結合）→ `4 / 2 * (4 / 2)`

ここまでを実装して measure したら**まだ 1 行違った**:
...
`astutil.SimplifyParentheses` は `ParenExpr` を剥がすだけの関数ではなく、
**`a OP (b OP c)` を `(a OP b) OP c` に回転させる**（演算子が同じとき、
変化しなくなるまで繰り返す）。積の演算子は常に `*` なので、
**底自身が `*` なら鎖に溶けて括弧が 1 つも残らない**:
...
同じ優先順位でも**演算子が違えば回転しない**。これは printer の規則では無いので、
探索 fixture を書いていなければ「括弧の規則は printer だ」で終わっていた。
`x*2` 系 5 形を fixture に足した。

#### `staticcheck-qf` を `divergent/` へ

QF1005 が閉じて、残った差は**続き 55 から README に書いてある 1 hunk だけ**:
上流の QF1004 は `s "strings"` を `strings.ReplaceAll(...)` に書き換えて
**import を足さない**ので、**コンパイルが通らない**コードを書く。
guff は `s.ReplaceAll(...)` を書く。

`pending` は「いつか閉じる穴」の台帳で、**閉じたら赤くなる**。
この hunk は**閉じてはいけない**（閉じたら `--fix` がユーザのビルドを壊し始める）。
続き 72 で `divergent/` が「上流も書く」ケースを持てるようになったので、そこへ移した。
3 つ目のエントリで、**性質は前の 2 つと違う** ——
`revive` / `parens` は**上流が触らない行**を書くが、これは
**上流と同じ行に違うバイトを書く**。成功行の文言もそれに合わせた
（`rewrites 0 thing(s) upstream leaves alone` は台帳のバグに読める）。
...
| pending | 17 | **16** |
| divergent | 2 | **3** |
| `staticcheck-qf` の差 | 4 行 | **1 hunk（意図的）** |
| golden `staticcheck-qf` キー | 175 | **180** |
| QF1005 の unit 検出数 | 6 | **33** |
| workspace テスト | 3,275 | **3,276** |

golden 193/193、269 スイート / 3,276 テスト。

#### 残り

pending 16 件は全部が書き足りない方向のまま。`divergent/` は 3 件になり、
**3 件が 3 通りの形**になった —— 上流が何も書かない（`revive`）、
...
### 2026-08-28（続き 85）— `go/doc/comment` の穴を測った。**「9 行」ではなく「整形されていない入力の 88%」だった**

続き 76 で gocritic の残り 9 行を `format_doc_comment` の no-op に帰属させ、
続き 81 でも同じものを据え置いた。どちらも**「9 行」として書いた**。
移植を検討する前に測ったら、その枠組みのほうが間違っていた。

#### 測り方

`crates/` と `compat/` の `.go` **3,912 ファイル**。
`gofmt` が読めるものについて 2 通り:

1. **そのまま**（既に gofmt 済み）に `guff fmt`
2. 行コメントの `// ` から**空白を 1 つ落として**から `guff fmt` と `gofmt -w` を比較
   —— gofmt が doc comment では復元し、関数本体では放置する形

```
既に gofmt 済みの入力:  3,907 件中 差は 0 件
空白を落とした入力:     1,638 件中 差は 1,455 件（88%）
```

代表例:

```
gofmt: // This comment has correct spelling.
guff:  //This comment has correct spelling.
```

#### 0 件のほうが、穴が見えない理由

`printer/comment.rs` の STATUS はこう書いている ——
**「On already-gofmt'd corpora this is usually idempotent」**。本当だった。

ただし「コーパスがほぼ全部 gofmt 済みだから」ではない。数えたら:

```
追跡下の .go            1,475 件
うち gofmt 非クリーン      215 件（15%）
うち // コメントで差が出るもの  十数件
```

**未整形のファイルはむしろ普通にある。** ゲートに見えないのは別の理由で、
**条件が 2 つ揃ったときだけ**乖離が現れるから:

1. gofmt が正規化する doc comment を持っている、**かつ**
2. **linter の fix が 1 つ以上当たる** —— そのとき初めて `editsByPath` に
   キーが立ち、ファイルが読み直されて gofmt され、書き戻される（続き 71）

fix が当たらない未整形 fixture は、どちらのツールも書かないので差が出ない。
`extras.go` は**両方が成り立つ唯一のケース**で、だからあの 1 件だけが気付いていた。

#### gocritic 1 件より重い理由

- **`guff fmt` を整形されていないコードに掛けるのは、フォーマッタの存在理由そのもの。**
  クリーンな入力での冪等性が、まさにフォーマッタが要る場面の穴を隠している。
- **`guff --fix` は適用後に gofmt する。** doc comment が正規形でないファイルに
  linter の fix が落ちると、guff と golangci は**違うバイトを書く** ——
  整形の好みではなく fix tier の主題。

#### それでも部分実装はしない

続き 76 で「空白を足すだけ」を退けた理由は変わらない ——
gofmt の実際の挙動は往復変換で、`//  two spaces` → `// two spaces`、
`//\ttabbed` → `// tabbed`、そして**空の `//` doc comment は削除される**。
部分規則は doc comment 内のインデント済みコードブロックを壊し、
**既知の乖離 1 つを 2 つに増やす**。
変わったのは**取引の価値の側**であって、危険の側ではない。

#### fixture は足さない

「先に測るための fixture を足す」も考えたが半分冗長だった ——
**`gocritic` の `extras.go` が既にその fixture**で、
9 行の pending はこの穴そのもの。2 つ目は同じものを 2 回測るだけ。

測れていないのは**広がり**（88%）で、
fix tier のケースは**ケース単位のバイト一致**を訊くので広がりを表現できない。
だからこの数字は fixture ではなく**この記録に**置く。
...
### 2026-08-28（続き 89）— `goimports` の pending を 22 行で閉じた。**5,608 ファイルのコーパスは、前後で 1 バイトも変わらなかった**

続き 88 で `compat/fmt` に置いた唯一の pending。**2 つの独立した欠陥**だった。

#### 台帳は設計どおり「直ったから」落ちた

```
goimports-local: guff now matches upstream — delete .../pending/goimports-local.go
```

pending は**両方向に失敗する**（続き 88）。**guff が正しくなった日に落ちる**性質が
実際に発火したのはこれが初めて。allowlist なら黙って通していた。

#### 欠陥 B —— 原因はタブ 1 文字

3 つの stdlib import を 1 ブロックで並べ替えると、guff は**同じグループの 2 つの間に空行**を入れていた。
...
**3 つとも `import_group` 0** なので、グループ判定では説明できない ——
つまり emit ループ（グループ変化で空行を入れる）は**無実**だと分かる。

`.why` に「読むだけでは足りない。`reconstruct` を instrument しろ」と書いて、そのとおりにしたら 1 回で出た:

```
DBG path="bytes" src_range=(34,42) bytes="\"bytes\""
DBG path="fmt"   src_range=(21,28) bytes="\"fmt\"\n\t"
DBG path="os"    src_range=(28,34) bytes="\"os\"\n\t"
```
...
**ブロックの末尾でない spec は `"fmt"\n\t`（改行＋次行のインデント）で終わる範囲を持つ。**
ループは最初の一歩で**タブ**を見て止まり、**改行も落とさない**。
呼び出し側が自分の `\n` を足すと、**タブ 1 つだけの行**ができ、
`go/printer` がそれを空行に詰める。

`\t` と ` ` を剥がす対象に足すだけ（1 行）。
**3 つ必要だったのも、順序に依存したのも**これで説明がつく ——
`\n\t` を持つのは末尾でない spec だけで、並べ替えがそれを末尾から動かしたときだけ出る。

#### 欠陥 A —— 早期 return に述語が 1 つ足りなかった
...
`import_runs_need_sort` は**並び順しか見ない**。上流の goimports は
`addImportSpaces` を**常に**走らせるので、**正しく並んでいるが区切りが無い**ブロックにも空行を足す。
`import_runs_need_group_blanks` を足した。

**逆向きは足していない。** 同じグループの間にユーザが書いた空行を、
上流は**保存する**（本物の `goimports` バイナリで確認した）。
対称に書くと既存テスト `preserves_user_blank_line_within_group` が落ちる ——
述語の doc comment にそう書いた。

#### 測定 —— **コーパスは前後で 1 バイトも変わらなかった**

| GOROOT 5,608 ファイル（`fmt_diff.py --formatter goimports`） | 修正前 | 修正後 |
|---|---|---|
| 一致 | 5,460 | 5,460 |
| 乖離 | 10 | 10（**ファイル集合まで同一**） |
| native_error | 138 | 138 |

**本物の `goimports` と 5,608 ファイル比べて、2 つのビルドが区別できない。**
手で確かめた 5 つの形はすべて変わっているのに。
理由は続き 87・88 とまったく同じ ——
**ディスク上のファイルは全部すでに正しくグループ分けされている**ので、
再グループ化の経路にも区切り追加の経路にも**一度も入らない**。
残る 10 件は元から乖離していた生成ファイル（`zsyscall_*` / `zdefaultcc`）。

prometheus は 725/725。`compat/fmt` は **13 → 15 ケース / 15 一致 / pending 0**。

#### 3 つの欠陥に共通する形

| 続き | 隠していたもの | ゲートの直し方 |
|---|---|---|
| 87 | コーパスが全部 gofmt 済み | `--unformat` で**入力を崩してから**両ツールに渡す |
| 88 | `fmt` を上流の `fmt` と比べるものが無かった | `compat/fmt` tier |
| 89 | コーパスが全部すでに正しくグループ分け | ケースを**出力形でない入力**で書く |

**出力形で保存されたコーパスは、その形への変換を検査できない。**

---

### 2026-08-28（続き 90）— `gofmt -s` を移植した。**既定設定が Go ツールチェーンを要らなくなった**

続き 88 で `formatters.settings.gofmt.simplify` の既定を `true` に直した。
そこまでは正しいが、**guff はその `-s` をシステムの `gofmt` に投げていた**。

#### 直す前に起きていたこと

既定 config（`formatters: {enable: [gofmt]}`）で **`gofmt` が PATH に無い**と:

```
guff fmt: (gofmt) formatting <standard input>: gofmt: No such file or directory (os error 2)
```
—— **警告を stderr に出して、ファイルを未整形のまま通し、exit 0。**

これは上流の `MetaFormatter.Format` の**エラー経路**（警告を出して入力を返す）と同じ形だが、
**上流はその経路に入らない**。golangci-lint は subprocess を使わないので、
`gofmt` の有無に関係なく整形する。**Go ツールチェーンの無い環境で、既定設定が何もしない。**

#### 移植したもの

| 上流 | 行 | |
|---|---|---|
| `cmd/gofmt/simplify.go` | 169 | **した** → `crates/guff-ast/src/simplify.rs` |
| `cmd/gofmt/rewrite.go` の `match(nil, …)` | のうち | **した**（同じファイル内） |
| `cmd/gofmt/rewrite.go` の `-r` 一式 | 309 | **しない** → `crate::gofmt` は今も subprocess |

**walk は独立に書いた。** `crate::walk` は読み取り専用、`crate::stamp` の可変走査は流用できない ——
simplifier は `ast.Walk` の**「nil を返して降りるのをやめる」**が要る
（簡約済みの composite literal の子を二度歩いてはいけない）。
stamp にその概念は無く、混ぜるとどちらかが壊れる。`go/ast/walk.go` を case ごとに写した。

**`match` も既存の式比較に寄せなかった。** `parser` の `expr_eq_shallow` は
**別の問い**（この仮引数は型をまとめてよいか）に答える関数で、
寄せると**正しい移植を別の移植の近似で上書きする**（続き 78 の教訓）。

#### 書きかけた近似を 1 つ消した

`match` の `FuncLit` 腕に「本文の文リストを長さで比べる」を書きかけて、
「どうせ到達しない」と注釈を付けた —— **それは根拠ではなく言い訳**。
`match` に渡るのは**型式だけ**（composite literal の要素型と内側の型）で、
関数リテラルは型ではないので**本当に到達しない**。
なので**腕ごと消して `_ => false` に落とした** —— 万一到達しても
「省略しない」という**安全側**に倒れる。近似を残すより短く、正しい。

#### 測定

| | main (81b8ff6a) | 今回 |
|---|---|---|
| GOROOT `gofmt -s`（`fmt_diff.py --simplify`） | 5,577 一致 / **31 乖離** | **5,608 / 0** |
| prometheus `gofmt -s` | — | **725 / 0** |
| GOROOT 素の `gofmt` | 5,608 / 0 | **5,608 / 0**（劣化なし） |
| 上流の `//gofmt -s` testdata | — | **4 / 4** |
| 上流の素の gofmt testdata | — | **10 / 10** |
| 手書きの形 × 本物の `gofmt -s` | — | **15 / 15** |
| `compat/fmt` | 15 / 15 | **15 / 15**（gofmt が native 経路に移っても同じバイト） |

**コーパスの数字の読み方に注意**（続き 87・88・89 と同じ）:
GOROOT も prometheus も**既に `-s` 済み**なので、5,608/5,608 が言うのは主に
**劣化していないこと**。実際の網羅は
**`-s` が実際に効く 31 ファイル**と、上流が**この機能のために持っている 4 対**と、
**分岐ごとの手書き 1 本ずつ**にある。

うち 2 本は「値だけ実装」でも他が全部通ってしまう形なので明示的に置いた ——
**map の *キー* 型の省略**（上流では別の呼び出し）と、
**`&T{}` の腕**（フィールドを消すのではなく要素そのものを置き換える）。
...
### 2026-08-28（続き 93）— `require-pkg-doc`。**「何が要るか」と書いておいた据え置きは、そのまま実装手順だった**

続き 92 で据え置きの理由を規則ごとに書いた。`require-pkg-doc` の欄はこう:

> **doc が無いファイルも含めた** package 名の位置。
> 今の `pkg_docs` は doc があるファイルしか記録しない。

**それがそのまま実装だった。** 新しい map を 1 つ足しただけで済んだ ——
`package name → (最初のファイルの package 識別子の位置, どれか 1 つでも doc を持つか)`。

#### この規則だけが「何も無かったファイル」に報告する

他の規則は「見つけた doc」に報告する。`require-pkg-doc` は
**doc が 1 つも無いことを、doc の無いファイルの package 節に報告する** ——
だから `pkg_docs`（doc を持つファイルしか記録しない）では足りない。
`fs[0].Name.Pos()`、つまり**最初の適用ファイル**の package 識別子。

#### 隣のコードから継承したら間違えていた 2 つ

| | `pkg-doc` | `require-pkg-doc` |
|---|---|---|
| `main` / `main_test` | **飛ばす**（command package は "Package main" で始まらなくてよい） | **飛ばさない** |
| `_test.go` | 除外（`include-tests: false`） | 除外（同じく `false`） |

`main` の非対称は既存コードのすぐ隣にある。**継承せずに測った** ——
両ツールとも `package main` に報告する。

`_test.go` は続き 92 で偽陰性を出した軸そのものなので、
**今度は上流の pin を先に読んでから**書いた。`no-unused-link` は `true`、
これは `false` —— **同じ linter の中で逆**。

#### 5 形すべてで一致 —— ただし「一致」だけでは見ない

| 形 | 結果 |
|---|---|
| どのファイルにも doc が無い | 報告（**最初のファイルだけ**） |
| 2 番目のファイルに doc | 沈黙（1 つあれば package は満たされる） |
| `_test.go` にだけ doc | **報告**（テストは数えない） |
| 外部 `pkg_test`・doc 無し | 沈黙（適用ファイルが無い） |
| `package main`・doc 無し | **報告** |

**3 形が実際に報告することを確かめた。** 「両方一致」は
**両方とも黙っている**でも成り立つので、出力そのものを見た（続き 30 以来の形）。

#### golden ケース

`nodoc/`（doc 無し 2 ファイル + doc 付き `c_test.go`）と
`hasdoc/`（1 ファイル目に doc 無し・2 ファイル目に doc）。
**記録は 1 キーだけ** —— それが「最初のファイルだけ」「1 つあれば足りる」
「テストは数えない」の 3 つを同時に固定する。
修正前の binary では `missing=1` で落ちる。

#### 残り

`default: all` の 9 規則のうち **6 つ**が動くようになった。
残り 3 つの「何が要るか」は続き 92 のまま:
...
### 2026-08-28（続き 97）— `max-len`。**測っているのはソースの行ではなく、印字し直した行**

続き 87 で `go/doc/comment` の `Printer` を移植したので、必要なものは揃っていた。

#### 手順が「長さを数える」だけではない

```
Code ブロックを落とす → 残りを Printer.Comment で印字し直す
  → 行に割る → rune 数で比べる
```

**印字し直した行**なので、パーサが字下げを付け直すブロックは付け直した形で測る。
ソースの行を数える実装は、この 1 点で違う答えを出す。

`Doc{Content: …}` には **Links を入れない**ので、Printer が末尾に付ける
link 定義ブロックは出ない。上流はこの数行手前で `linkDefsMap` を組み立てて
**一度も読まない** —— 古い実装の残骸で、移植すると
**もう存在しない行を除外する**コードになる。写さなかった。

#### 報告位置は「その行の `ast.Comment`」

`// ` ＋ 印字行 と一致する `ast.Comment` を**リストを消費しながら**探す。
消費するので、同じ行が 2 回出てくると**別々の行**として報告される。
一致しなければ group 全体に落ちる —— `/*…*/` は
**group 全体が 1 つの `ast.Comment`** で位置が `/*` なので必ずこちら。

#### `pending.dedup()` は防御ではなく必須だった

`/*…*/` の中に同じ長すぎる行が 2 本あると、上流は**同じ位置に同じ文言を 2 件**出す。
消そうかと思ったので**測った**: golangci-lint は
（`uniq-by-line: false` ／ `max-same-issues: 0` でも）**1 件しか出さない**。
だから guff も畳む。コメントにその測定を書いた。

#### golden 2 本を同じ fixture に当てた

`godoclint-max-len`（8 キー・既定 77）と
`godoclint-max-len-length`（15 キー・`length: 39`）。
**数値が文言に入る**ので、限界値を焼き込んだ実装では 2 本同時には通らない。

**rune 数であることを両側から固定した**: 40 文字 120 バイトの CJK 行は
77 では**報告されず**、39 では **`(40 > 39)`** として報告される ——
バイト数なら 120 になる。

他に fixture が分けているもの: ちょうど 77 の行（報告しない）、
字下げ Code ブロック（印字前に落ちる）、`^\+kubebuilder:`（pin された除外）、
繰り返し行 2 本、`/*…*/` の fallback、doc を持たない symbol 経由の親 doc、
`_test.go`（pin は **true**）。どちらも修正前の binary では全件 missing。
