...
### 1.1 V8-1 — `visit_masked` にアーム別カウンタを足した（**DONE**）
...
### V8-2 — `format_checks` 0.69s の中を測る（**未着手 / 最優先**）
...
### V8-3 — wide linear scan の 3 本（**未着手 / CPU 向け**）
...
### V8-4 — ruff 方式の融合トラバーサル（**未着手 / 大物・CPU 向け**）
...
### V8-5 — 依存の型情報を遅延化して RSS を下げる（**未着手 / RSS 向け**）
...
### V8-6 — `--watch` に型 / SSA を保持させる（**未着手 / 特大**）
