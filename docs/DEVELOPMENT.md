...
> 最終更新: 2026-07-16。ワークスペース全体 **1900+ tests green**（`guff-revive` extended rules 計 **100 rules** + `linters.settings.revive` / `dupl` / `misspell` / `godot` / `godox` / `dupword` / **`depguard` / `gomoddirectives` / `gomodguard` / `wrapcheck` / `exhaustive` / `musttag` / `loggercheck` / `sloglint` / `testifylint`（22 checkers）** YAML 配線）。
...
- **完了条件**: 各 linter に bad/ok testdata、`go vet`/golangci 相当の指摘、レジストリ登録。
  ...
  **DEFERRED（同タスク内の残り）**:
  ...
  - ~~`testifylint`~~ → **部分完了**（2026-07-16；suite-dont-use-pkg / suite-extra-assert-call / suite-subtest-run 追加で計 22 checker；go-require / mock-expect / require-error / suite-broken-parallel / suite-method-signature / suite-thelper・SuggestedFix・formatter printf CheckPrintf は DEFERRED）
