...
> 最終更新: 2026-07-15。ワークスペース全体 **1896+ tests green**（本セッションで `guff-style` 6 tests 追加）。
...
| `guff-style` | ✅ **3**（copyloopvar / usetesting / usestdlibvars） | perfsprint / goconst・R14 バンドル残りは **DEFERRED** |
...
| プリセット | `standard`/`fast`/`all`/`none`。ただし `standard`==`all`（5 系統）。追加系は `--enable`（forcetypeassert/nilnil/makezero/errname/err113/durationcheck/errorlint/wrapcheck/noctx/fatcontext/copyloopvar/usetesting/usestdlibvars） | 100+ linter を跨ぐ本来の `all`/`fast`/カテゴリプリセットに未対応 |
...
- **進捗メモ (2026-07-15)**:
  ...
  **DEFERRED（同タスク内の残り）**:
  ...
  - `usestdlibvars` の optional テーブル / `usetesting`・`copyloopvar` の settings 配線
  - `errchkjson` / `rowserrcheck` / bodyclose / contextcheck / sqlclosecheck / gosec / perfsprint / goconst ほか
    （R13 続きセッションで数個ずつ）
...
- `guff-style` バンドル: funlen, gocyclo, gocognit, cyclop, nestif, lll, whitespace, wsl, nlreturn,
  ...
  （クレート自体は R13 で新設済み。copyloopvar / usetesting / usestdlibvars 済。）
