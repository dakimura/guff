...
> 最終更新: 2026-07-16。ワークスペース全体 **1900+ tests green**（`guff-revive` extended rules 計 **100 rules** + `linters.settings.revive` / `dupl` / `misspell` / `godot` / `godox` / `dupword` / **`depguard` / `gomoddirectives` / `gomodguard` / `wrapcheck` / `exhaustive` / `musttag` / `loggercheck` / `sloglint` / `testifylint`（28 checkers） / `exptostd` / `modernize` / `gocritic`（29 checkers）** YAML 配線）。
...
| `guff-revive` | ✅ **1**（revive） | golint-default **23 rules** + extended **77 rules**（計 100）；`linters.settings.revive` YAML 配線済み（rules・arguments・global/per-rule severity・confidence・ignore-generated-header） |
...
- **完了条件**: 各 linter に bad/ok testdata、`go vet`/golangci 相当の指摘、レジストリ登録。
  ...
  4. `guff-style`（新）: **copyloopvar**（`check-alias` settings 済）/ **usetesting**（per-check flags settings 済）/
     **usestdlibvars**（HTTP method/status 既定オン + optional tables settings 済）
     ...
     / **modernize**（x/tools modernize；any / plusbuild / forvar / rangeint / minmax / fmtappendf / omitzero / slicessort；`linters.settings.modernize.disable` 済）
     / **gocritic**（go-critic；**29** checkers: appendAssign / argOrder / assignOp / badCall / badCond / captLocal / defaultCaseOrder / dupArg / dupBranchBody / dupCase / dupSubExpr / elseif / exitAfterDefer / flagDeref / flagName / ifElseChain / mapKey / newDeref / offBy1 / regexpMust / singleCaseSwitch / sloppyLen / switchTrue / typeSwitchVar / underef / unlambda / unslice / valSwap / wrapperFunc；`linters.settings.gocritic` enable-all/disable-all/enabled-checks/disabled-checks 済）
  ...
  **DEFERRED（同タスク内の残り）**:
  ...
  - ~~`modernize`（コア checker）~~ → **完了**（2026-07-16；any / plusbuild / forvar / rangeint / minmax / fmtappendf / omitzero / slicessort + `disable` settings。atomictypes / embedlit / errorsastype / mapsloop / newexpr / reflecttypefor / slicesbackward / slicescontains / stditerators / stringscut* / stringsbuilder / testingcontext / unsafefuncs / waitgroupgo と rangeint/minmax 完全パリティは DEFERRED）
  - ~~`gocritic`（コア checker）~~ → **完了**（2026-07-16；初回 18 → **29** checks + enable-all/disabled-checks。残 caseOrder / codegenComment / commentFormatting / deprecatedComment / sloppyTypeAssert 等・per-check settings・SuggestedFix・型完全パリティは DEFERRED）
