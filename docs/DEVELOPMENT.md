...
> 最終更新: 2026-07-16。ワークスペース全体 **1900+ tests green**（`guff-revive` extended rules 計 **100 rules** + `linters.settings.revive` / `dupl` / `misspell` / `godot` / `godox` / `dupword` / **`depguard` / `gomoddirectives` / `gomodguard` / `wrapcheck` / `exhaustive` / `musttag`** YAML 配線）。
...
- **完了条件**: 各 linter に bad/ok testdata、`go vet`/golangci 相当の指摘、レジストリ登録。
  ...
  **DEFERRED（同タスク内の残り）**:
  ...
  - gosec / loggercheck / sloglint / testifylint ほか（R13 続きセッションで数個ずつ）
