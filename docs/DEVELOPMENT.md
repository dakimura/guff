...
> 最終更新: 2026-07-15。ワークスペース全体 **1900+ tests green**（本セッションで `guff-style` に whitespace / nlreturn / mnd を追加）。
...
| `guff-style` | ✅ **20**（copyloopvar / usetesting / usestdlibvars / perfsprint / goconst / dogsled / asciicheck / goprintffuncname / funlen / gocyclo / lll / gocognit / nestif / cyclop / nakedret / nosprintfhostport / predeclared / whitespace / nlreturn / mnd） | R14 バンドル残り（wsl/prealloc/tagalign 等）・settings は **DEFERRED** |
...
- **完了条件**: 各 linter に bad/ok testdata、`go vet`/golangci 相当の指摘、レジストリ登録。
  ...
  4. `guff-style`（新）: **copyloopvar** / **usetesting** / **usestdlibvars**（HTTP method/status 既定オン）
     ...
     / **nakedret** / **nosprintfhostport** / **predeclared** / **whitespace** / **nlreturn** / **mnd**（R14 で追加）
...
- `guff-style` バンドル: funlen, gocyclo, gocognit, cyclop, nestif, lll, whitespace, wsl, nlreturn,
  ...
  **whitespace**（multi-if/multi-func 既定オフ）/ **nlreturn**（block-size=1）/ **mnd**（全 checks・0/1 無視）済。
  ...
  whitespace SuggestedFix / multi-*・nlreturn SuggestedFix・mnd settings・**wsl** / **prealloc** /
  **tagalign** は DEFERRED。）
