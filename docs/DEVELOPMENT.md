...
| **Formatters** | `guff-fmt` | `guff fmt`（gofmt / gofumpt / goimports / gci / golines；swaggo は R15 DEFERRED） |
...
| `guff-fmt` | ✅ gofmt / gofumpt / goimports / gci / golines（`guff fmt`） | swaggo は DEFERRED |
...
- **完了条件**: 各 linter に bad/ok testdata、`go vet`/golangci 相当の指摘、レジストリ登録。
  ...
  2. `guff-error`: errname / err113 / durationcheck / **errorlint** / **wrapcheck** / **errchkjson** /
     **rowserrcheck**
     ...
     rowserrcheck `packages` settings 配線済み；AST 近似・SSA 完全パリティは DEFERRED）
  3. `guff-context`: **noctx**（AST call-name; upstream は buildssa）/ **fatcontext** / **bodyclose**（AST 近似; upstream は buildssa；`check-consumption` settings 済）/ **sqlclosecheck**（AST 近似; upstream は buildssa defer-only；Close 欠落 + non-defer Close）
  ...
  **DEFERRED（同タスク内の残り）**:
   ...
   - ~~`rowserrcheck`~~ → **完了**（2026-07-17；jingyugao/rowserrcheck AST 近似。`database/sql` 常時 + `linters.settings.rowserrcheck.packages`。関数戻り `*…Rows` スキップ。SSA Phi/closure/FieldAddr 完全パリティは DEFERRED）
   - ~~`bodyclose`~~ → **完了**（2026-07-17；timakin/bodyclose AST 近似。`*http.Response` 追跡 + `.Body.Close()` / defer；関数戻り `*http.Response` スキップ；`httptest.ResponseRecorder.Result` スキップ；`linters.settings.bodyclose.check-consumption` 配線。SSA Phi / closure / FieldAddr / `io.Closer` ChangeInterface 完全パリティは DEFERRED）
   - ~~`sqlclosecheck`~~ → **完了**（2026-07-17；ryanrolds/sqlclosecheck defer-only AST 近似。`*sql.Rows`/`*sql.Stmt`/`*sqlx.NamedStmt`/pgx Rows の Close 欠落 + non-defer Close；関数戻りスキップ；引数渡しは handled；設定キー無し。SSA Phi / closure / FieldAddr 完全パリティは DEFERRED）
   - contextcheck — SSA（→ R17）
   ...
   - ~~`clickhouselint`~~ → **完了**（2026-07-17；ClickHouse/clickhouse-go-linter；`chrowserr`（`Rows.Next` 後の `Err` 欠落）+ `chbatchclose`（`Batch` の `defer Close` / return / blank 代入）；intra-procedural；設定キー無し。`CH_GO_LINTER_DEBUG` は DEFERRED）
   - ~~`wsl_v5`~~ → **完了**（2026-07-17；bombsimon/wsl/v5；デフォルトチェック + `linters.settings.wsl_v5`。SuggestedFix・after-* / assign-exclusive / assign-expr / cuddle-group・decl grouping・Lock/Unlock・真の error Implements・case-max-lines trailing は DEFERRED）
...
- **進捗メモ (2026-07-17)**:
  ...
  - **golines** をシステム `golines` バイナリ経由で実装（stdin → stdout；`github.com/golangci/golines`）。
  - `formatters.enable` / `formatters.settings.gofmt`（`simplify` / `rewrite-rules`）/
    ...
    `formatters.settings.golines`（`max-len` / `tab-len` / `shorten-comments` /
    `reformat-tags` / `chain-split-dots`；`--base-formatter=gofmt` でネスト goimports 回避）/
  ...
  - enable 順でチェーン（gofmt → gofumpt → goimports → gci → golines 等）。
  - generated ファイル（`Code generated` … `DO NOT EDIT`）をスキップ。
  - **DEFERRED**: swaggo、`guff run` 時の formatter 診断、
