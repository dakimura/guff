...
- **完了条件**: 各 linter に bad/ok testdata、`go vet`/golangci 相当の指摘、レジストリ登録。
  ...
  4. `guff-style`（新）: **copyloopvar**（`check-alias` settings 済）/ **usetesting**（per-check flags settings 済）/
     **usestdlibvars**（HTTP method/status 既定オン + optional tables settings 済）
     ...
     / **gocritic**（go-critic；**67** checkers: default 34 + enable-all extras 33: deferUnlambda / emptyDecl / emptyFallthrough / emptyStringTest / initClause / nilValReturn / octalLiteral / yodaStyleExpr / builtinShadow / builtinShadowDecl / commentedOutImport / dupImport / filepathJoin / paramTypeCombine / rangeAppendAll / weakCond / dupOption / methodExprCall / rangeExprCopy / regexpPattern / sortSlice / sqlQuery / typeAssertChain / **badRegexp** / **truncateCmp** / **typeDefFirst** / **deferInLoop** / **hexLiteral** / **nestingReduce** / **todoCommentWithoutDetail** / **docStub** / **unnecessaryBlock** / **sloppyReassign**；`linters.settings.gocritic` enable-all/disable-all/enabled-checks/disabled-checks 済）
  ...
  **DEFERRED（同タスク内の残り）**:
   ...
   - ~~`gocritic`（コア + prometheus enable-all 主要 extras）~~ → **拡充**（2026-07-17；初回 18 → 29 → 34 → 42 → 50 → 57 → 58 → **67** checks + enable-all/disabled-checks。prometheus が有効のまま残す `truncateCmp` / `typeDefFirst` を含む。残 enable-all extras: `ruleguard` ほか未移植 default/opinionated；per-check settings・SuggestedFix・caseOrder 式 switch 重なりは DEFERRED）
