...
| プリセット | `standard`/`fast`/`all`/`none`。ただし `standard`==`all`（5 系統）。追加系は `--enable`（） | 100+ linter を跨ぐ本来の `all`/`fast`/カテゴリプリセットに未対応 |
...
- **完了条件**: 各 linter に bad/ok testdata、`go vet`/golangci 相当の指摘、レジストリ登録。
  ...
  **DEFERRED（同タスク内の残り）**:
   ...
   - gosec / unparam / iface / godoclint / wastedassign / spancheck ほか（R13 続きセッションで数個ずつ）
...
- `guff-style` バンドル: funlen, gocyclo, gocognit, cyclop, nestif, lll, whitespace, wsl, nlreturn,
  ...
  **wsl**（v4 既定 cuddle の主要ルール）/ **forbidigo**（既定 print 禁止 + settings）/ **reassign**（既定 EOF/Err* + `patterns` settings）/ **thelper**（t/f/b/tb begin/first/name + settings）済。
