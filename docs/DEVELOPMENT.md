...
> 最終更新: 2026-07-16。ワークスペース全体 **1900+ tests green**（`guff-revive` extended rules 計 **100 rules** + `linters.settings.revive` / `dupl` / `misspell` / `godot` / `godox` / `dupword` / **`depguard` / `gomoddirectives` / `gomodguard` / `wrapcheck` / `exhaustive` / `musttag` / `loggercheck` / `sloglint` / `testifylint`（28 checkers） / `exptostd` / `modernize` / `gocritic`（34 checkers）** YAML 配線）。
...
- **完了条件**: 各 linter に bad/ok testdata、`go vet`/golangci 相当の指摘、レジストリ登録。
  ...
  4. `guff-style`（新）: **copyloopvar**（`check-alias` settings 済）/ **usetesting**（per-check flags settings 済）/
     **usestdlibvars**（HTTP method/status 既定オン + optional tables settings 済）
     ...
     / **gocritic**（go-critic；**34** checkers: appendAssign / argOrder / assignOp / badCall / badCond / captLocal / caseOrder / codegenComment / commentFormatting / defaultCaseOrder / deprecatedComment / dupArg / dupBranchBody / dupCase / dupSubExpr / elseif / exitAfterDefer / flagDeref / flagName / ifElseChain / mapKey / newDeref / offBy1 / regexpMust / singleCaseSwitch / sloppyLen / sloppyTypeAssert / switchTrue / typeSwitchVar / underef / unlambda / unslice / valSwap / wrapperFunc；`linters.settings.gocritic` enable-all/disable-all/enabled-checks/disabled-checks 済）
  ...
  **DEFERRED（同タスク内の残り）**:
  ...
  - ~~`gocritic`（コア checker）~~ → **完了**（2026-07-16；初回 18 → 29 → **34** checks + enable-all/disabled-checks。残 enable-all extras・per-check settings・SuggestedFix・caseOrder 式 switch 重なりは DEFERRED）
