...
- **完了条件**: 各 linter に bad/ok testdata、`go vet`/golangci 相当の指摘、レジストリ登録。
  ...
  4. `guff-style`（新）: **copyloopvar**（`check-alias` settings 済）/ **usetesting**（per-check flags settings 済）/
     **usestdlibvars**（HTTP method/status 既定オン + optional tables settings 済）
     ...
     / **gosec**（securego/gosec；初回ルール G103/G401/G404/G405/G406/G501–G507；`linters.settings.gosec` の `includes`/`excludes` 配線済。残ルール・severity/confidence/config は DEFERRED）
  ...
  **DEFERRED（同タスク内の残り）**:
   ...
   - ~~`gosec`（初回バッチ）~~ → **部分完了**（2026-07-17；G103/G401/G404/G405/G406/G501–G507 + `includes`/`excludes`。残ルールは DEFERRED）
