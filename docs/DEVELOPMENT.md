...
| `guff-context` | ✅ **3**（noctx / fatcontext / **bodyclose**） | contextcheck / sqlclosecheck 等は **DEFERRED**（SSA → R17）。**bodyclose** AST 近似（`check-consumption` settings 済；SSA 完全パリティは DEFERRED） |
...
- **完了条件**: 各 linter に bad/ok testdata、`go vet`/golangci 相当の指摘、レジストリ登録。
  ...
  3. `guff-context`: **noctx**（AST call-name; upstream は buildssa）/ **fatcontext** / **bodyclose**（AST 近似; upstream は buildssa；`check-consumption` settings 済）
  ...
  **DEFERRED（同タスク内の残り）**:
   ...
   - contextcheck / sqlclosecheck — SSA（→ R17）
