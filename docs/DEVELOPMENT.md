...
補足（2026-07-17）: `gocritic` enable-all extras に **`badLock`** / **`externalErrorReassign`** / **`uncheckedInlineErr`** / **`boolExprSimplify`** を追加し、計 **106** checker（default 34 + extras 72）。`boolExprSimplify` は removeIncDec / foldRanges まで実装済み。残 `regexpSimplify`、`ruleguard` ホストは DEFERRED。
...
- **完了条件**: 各 linter に bad/ok testdata、`go vet`/golangci 相当の指摘、レジストリ登録。
  ...
  **DEFERRED（同タスク内の残り）**:
   ...
   - ~~`gocritic`（コア + prometheus enable-all 主要 extras）~~ → **拡充**（2026-07-17；初回 18 → … → 101 → 102 → **106** checks + enable-all/disabled-checks。`badLock` / `externalErrorReassign` / `uncheckedInlineErr` / `boolExprSimplify`（doubleNegation / invertComparison / negatedEquals / combineChecks / **removeIncDec** / **foldRanges**）を追加。残 enable-all extras: `ruleguard` ホスト / `regexpSimplify`；per-check settings・SuggestedFix・caseOrder 式 switch 重なり・redundantSprint 真の Stringer Implements は DEFERRED）
     ...
     追記（2026-07-17）: `boolExprSimplify` に **removeIncDec** / **foldRanges** を追加（`x < y+1` → `x <= y`、`x > 10 && x < 12` → `x == 11` 等；`!(x >= y+1)` は post-order で `x <= y`）。SkipChilds・SideEffectFree 完全パリティは DEFERRED。残 enable-all extras は `ruleguard` / `regexpSimplify`。
