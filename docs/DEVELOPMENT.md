...
| **Linters** | `guff-staticcheck`, `guff-govet`, `guff-errcheck`, `guff-ineffassign`, `guff-unused`, `guff-gostaticanalysis`, `guff-error`, `guff-context`, `guff-style`, `guff-comment`, `guff-import` | 各 linter の Analyzer 群 |
...
guff-ast / guff-constant / guff-version*
  ...
  → guff-runner
  → guff-{staticcheck,govet,errcheck,ineffassign,unused,gostaticanalysis,error,context,style,comment,import}
  → guff-lint
```
...
> 最終更新: 2026-07-15。ワークスペース全体 **1900+ tests green**（本セッションで `guff-import` に depguard / gomoddirectives / gomodguard を追加）。
...
| `guff-import` | ✅ **3**（depguard / gomoddirectives / gomodguard） | settings・gomodguard_v2 は **DEFERRED**；R14 残は revive/misspell/dupl |
...
- `guff-comment`: **godot**（declarations + period 既定）/ **godox**（TODO/BUG/FIXME）/ **dupword**
  ...
  フィルタは DEFERRED。
- `guff-import`: **depguard**（既定 `$gostd` only）/ **gomoddirectives**（既定: replace 禁止・retract に説明必須）/
  **gomodguard**（既定空設定＝指摘なし；blocked / local-replace はテストヘルパーで検証）。
