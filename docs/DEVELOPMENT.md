...
| `guff-revive` | ✅ **1**（revive） | golint-default **23 rules** + extended **21 rules**（計 44）；settings・残 extended rules は **DEFERRED** |
...
- `guff-revive`: **revive**（golint-default **23 rules** + extended **21 rules**: atomic / bare-return / bool-literal-in-expr / call-to-gc / cognitive-complexity / constant-logical-expr / cyclomatic / duplicated-imports / if-return / import-shadowing / string-of-int / struct-tag / time-date / time-equal / unchecked-type-assertion / unconditional-recursion / unhandled-error / unnecessary-format / unnecessary-stmt / use-errors-new / waitgroup-by-value）。
  `linters.settings.revive` YAML 配線と残 extended rules（add-constant, argument-limit, early-return, …）は DEFERRED。
...
| 日付 | 内容 |
|------|------|
| 2026-07-15 | **R14 続き**: `guff-revive` に extended revive rules **7** 件（cognitive-complexity / constant-logical-expr / import-shadowing / struct-tag / time-date / unhandled-error / unnecessary-stmt）を追加。golint-default 23 + extended 21 = **44 rules**。`linters.settings.revive` と残 extended rules は DEFERRED |
