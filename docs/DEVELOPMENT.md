...
- **テスト**: `python3 -m unittest discover -s compat/tests` + `./compat/smoke.sh` + `./compat/reject/run.sh`（上流が起動を拒む config を guff も拒むか。finding 集合の tier では表現できない側 —— `compat/reject/README.md`）。
...
位置空間の生オフセットのままでは追えない）。

**依存側の診断は `GUFF_DEBUG_SEED_ERRORS=1`。** seed の依存パッケージの型エラーは
ユーザのパッケージのものではないので報告しない —— が、「報告しない」は
「観測できない」とは別である。seed の依存が型検査に失敗すると、それを import する
ターゲットは不完全な型を見て、**ターゲット側のエラーがまったく別のものを名指しする**:
`pkg/cluster` の失敗が `manager.Manager has no field or method GetCache` として出る
(§4 の 2026-08-13)。1 行目のエラーを推測ではなく**読む**唯一の手段がこれ。
