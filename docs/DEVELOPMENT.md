...
> 最終更新: 2026-07-16。ワークスペース全体 **1900+ tests green**（`guff-revive` extended rules 計 **100 rules** + `linters.settings.revive` / `dupl` / `misspell` / `godot` / `godox` / `dupword` / **`depguard` / `gomoddirectives` / `gomodguard` / `wrapcheck` / `exhaustive` / `musttag` / `loggercheck` / `sloglint` / `testifylint`（27 checkers）** YAML 配線）。
...
- **完了条件**: 各 linter に bad/ok testdata、`go vet`/golangci 相当の指摘、レジストリ登録。
  ...
  **DEFERRED（同タスク内の残り）**:
  ...
  - ~~`testifylint`~~ → **部分完了**（2026-07-16；計 **27** checker；`go-require` + `ignore-http-handlers` 配線済；残り `mock-expect`・SuggestedFix・formatter printf CheckPrintf・require-error の WaitGroup.Go 文脈は go-require 側でカバー）
  - `rowserrcheck` / bodyclose / contextcheck / sqlclosecheck — SSA（→ R17）
  - gosec / testifylint 残り checker（mock-expect）/ gocritic / unparam ほか（R13 続きセッションで数個ずつ）
