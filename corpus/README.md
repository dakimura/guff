...
| zitadel/zitadel | yes (`.golangci.yaml`) | **The protobuf and OpenAPI code is generated at build time and committed nowhere, so upstream's whole report is one `typecheck` issue.** Measured 2026-09-22 at `v4.17.1`. `pkg/grpc/**` holds only the hand-written half of each package — `pkg/grpc/message/` is `message.go` alone, and it names `LocalizedMessage`, which the missing `message.pb.go` declares — and `openapi/handler.go` embeds `v2/zitadel/*`, which exists in no clone. Plain `go list ./...` fails outright (`pattern v2/zitadel/*: no matching files found`, 0 packages); with `-e`, **123 of 330 packages** fail to load, almost all through `DepsErrors` on `pkg/grpc/settings/v2` (111), `pkg/grpc/object/v3alpha` (67), `pkg/grpc/system` (34), `pkg/grpc/object` (23). golangci-lint 2.12.2 with the repository's own config ran for 26s and printed **1 issue, `typecheck`** (`pkg/grpc/message/message.go: undefined: LocalizedMessage`), which deletes every other issue in the run — ollama's shape, but the missing input is generated code rather than a front-end build. Not platform-bound: it fails the same on any host until the nx build runs `buf generate` over `buf.gen.yaml` — `go`, `go-grpc`, `grpc-gateway`, `openapiv2`, `validate`, and zitadel's **own** `authoption` / `zitadel` protoc plugins — none of which the entry schema has a field to run. (The config also sets `issues.new-from-rev: origin/main`.) |
| pingcap/tidb | yes (`.golangci.yml`, v2 on `v26.3.17`; **v1** on the candidate's `v8.5.7`) | **golangci-lint itself crashes on it, so there is no reference to be compatible with.** Measured 2026-09-22. The candidate row's `v8.5.7` (the LTS line) carries a v1 config (`disable-all`, `varcheck`, `deadcode`, no `version`) that golangci-lint v2 refuses (`unsupported version of the configuration: ""`); the newest tag, `v26.3.17` (2026-09-21), has a v2 config, so that is what was measured. All 782 packages load and type-check on darwin (`go build` fails only at link time in `tools/patch-go` — a `//go:linkname` into the runtime — and `pkg/plugin/conn_ip_example`, a plugin with no `main`). golangci-lint 2.12.2 (built with go1.26.2), run with the repository's own config over `./...`, **panicked in two runs out of two** — the adoption hunt and a rerun, exit 2, the same trace each time: a nil dereference inside go/types, `Checker.builtin` → `TypeParam.typeset` → `isString(nil)`, reached from `loadingPackage.loadImportedPackageWithFacts` (a dependency type-checked from source). A crash is not a finding set. (A third run was stopped under memory pressure.) |
| open-telemetry/opentelemetry-collector-contrib | yes (`.golangci.yml`) | **The harness cannot reach any of the code: the root module has no Go file at all.** Measured 2026-09-22 at `v0.159.0` from the tag's git tree (`git/trees?recursive=1`, not truncated). There are **346 `go.mod` files and no `go.work`**; the root `go.mod` exists, but all **7,621** `.go` files sit under the other **345** modules, so `./...` at the checkout root matches no package, and a nested module cannot be named from there (`directory prefix … does not contain main module`, as in the sibling `opentelemetry-collector` row). Upstream runs its targets module by module (the Makefile dispatches them through `$(FOR_GROUP_TARGET)` over module groups assembled into `ALL_MODS`); the entry schema runs one pattern in one directory. Same shape as opentelemetry-collector, one step further — that one at least reached `internal/statusutil`. |
