...
| grafana/tempo | yes (`.golangci.yml`) | **guff does not finish — its own defect, not the environment.** Measured 2026-09-08 at `v3.0.3`. The hunt timed out at 25m; golangci-lint lints the whole repo in **44s** (461 findings). Not size and not vendoring: with `vendor/` moved aside guff still times out, and gatekeeper — adopted clean — reaches 1622 transitive deps. It is **one dependency**. A five-line module whose only requirement is `github.com/bytedance/sonic` takes golangci **0s** and guff **>180s**, with a single trivial linter (`ineffassign`) on one package, so the cost is in loading/type-checking and not in any analyzer; v1.15.0 and v1.15.1 behave the same. gin, woodpecker, traefik and celestia-node also `require` sonic and stay clean because it is absent from their analysed graph (0 packages each), while tempo has 26. Re-adopt as soon as that loader cost is fixed — nothing about tempo itself blocks it. |
