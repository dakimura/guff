...
pub(crate) fn norm(arena: &TypeArena, xl: &TermList) -> TermList {
    ...
    for i in 0..xl.len() {
        ...
        for j in (i + 1)..xl.len() {
            ...
            let (u1, u2) = typeterm::union(arena, xi, xl[j]);
            if u2.is_none() {
...
/// `xl ∪ yl`.
pub(crate) fn union(arena: &TypeArena, xl: &TermList, yl: &TermList) -> TermList {
...
pub(crate) fn intersect(arena: &TypeArena, xl: &TermList, yl: &TermList) -> TermList {
    ...
    for &x in xl {
        for &y in yl {
            let r = typeterm::intersect(arena, x, y);
            if r.is_some() {
                rl.push(r);
            }
        }
...
pub(crate) fn equal(arena: &TypeArena, xl: &TermList, yl: &TermList) -> bool {
    subset_of(arena, xl, yl) && subset_of(arena, yl, xl)
}

/// Reports whether `t ∈ xl`.
#[allow(dead_code)] // used by predicates.go when ported.
pub(crate) fn includes(arena: &TypeArena, xl: &TermList, t: TypeId) -> bool {
    xl.iter().any(|&x| typeterm::includes(arena, x, t))
}

/// Reports whether `y ⊆ xl` — some term in `xl` contains `y`.
#[allow(dead_code)] // used by `subset_of` below; reachable from `equal`.
pub(crate) fn superset_of(arena: &TypeArena, xl: &TermList, y: Option<Term>) -> bool {
    xl.iter().any(|&x| typeterm::subset_of(arena, y, x))
}

/// Reports whether `xl ⊆ yl`.
#[allow(dead_code)] // used by `equal`.
pub(crate) fn subset_of(arena: &TypeArena, xl: &TermList, yl: &TermList) -> bool {
    if is_empty(yl) {
        return is_empty(xl);
    }
    xl.iter().all(|&x| superset_of(arena, yl, x))
}
