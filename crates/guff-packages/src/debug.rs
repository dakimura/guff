...
//!
//! Level 1 (the variable set to anything) prints the phase totals that have
//! always been there; those lines are byte-identical to before this module
...
//!
//! `guff-lint` reads the same variable with its own copy of this logic — the
//! two crates share no support crate, and `guff-analysis` already reads it
//! independently. Keep the level mapping in sync if it ever changes.
