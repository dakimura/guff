...
fn parse_v2_reassign_settings() {
    ...
    let cfg = parse_config_str(&contents).unwrap();
    let settings = LinterSettings::from_yaml(cfg.linter_settings_raw());
    assert_eq!(settings.reassign.patterns, vec![".*".to_string()]);
    let bag = settings.to_bag();
    let opts = bag
        .get::<guff_style::ReassignOptions>("reassign")
        .expect("reassign options");
    assert_eq!(opts.patterns, vec![".*".to_string()]);
}
...
fn parse_v2_maintidx_settings() {
    ...
    let cfg = parse_config_str(&contents).unwrap();
    let settings = LinterSettings::from_yaml(cfg.linter_settings_raw());
    assert_eq!(settings.maintidx.under, 100);
    let bag = settings.to_bag();
    let opts = bag
        .get::<guff_style::MaintidxOptions>("maintidx")
        .expect("maintidx options");
    assert_eq!(opts.under, 100);
}
...
fn parse_v2_nonamedreturns_settings() {
    ...
    let cfg = parse_config_str(&contents).unwrap();
    let settings = LinterSettings::from_yaml(cfg.linter_settings_raw());
    assert!(settings.nonamedreturns.report_error_in_defer);
    assert!(settings.nonamedreturns.allow_unused_named_returns);
    let bag = settings.to_bag();
    let opts = bag
        .get::<guff_style::NonamedreturnsOptions>("nonamedreturns")
        .expect("nonamedreturns options");
    assert!(opts.report_error_in_defer);
    assert!(opts.allow_unused_named_returns);
}
...
fn parse_v2_unqueryvet_settings() {
    ...
    let cfg = parse_config_str(&contents).unwrap();
    let settings = LinterSettings::from_yaml(cfg.linter_settings_raw());
    assert!(!settings.unqueryvet.check_aliased_wildcard);
    assert!(settings.unqueryvet.check_subqueries);
    assert_eq!(
        settings.unqueryvet.allowed_patterns,
        vec!["(?i)SELECT \\* FROM temp_.*".to_string()]
    );
    let bag = settings.to_bag();
    let opts = bag
        .get::<guff_style::UnqueryvetOptions>("unqueryvet")
        .expect("unqueryvet options");
    assert!(!opts.check_aliased_wildcard);
    assert!(opts.check_subqueries);
    assert_eq!(
        opts.allowed_patterns,
        vec!["(?i)SELECT \\* FROM temp_.*".to_string()]
    );
}
