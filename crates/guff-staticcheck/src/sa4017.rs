...
/// value — not ExprStmt discards, plus a `lparen -> CallExpr.Pos()` map.
///
/// The IR stamps a call instruction with the CallExpr's **lparen**, but
/// upstream reports the call node itself, whose position is the start of the
/// callee expression. Without the map every finding lands on the `(`.
    ...
    for file in pass.files() {
        preorder(NodeRef::File(file), |n| {
            if let NodeRef::CallExpr(c) = n {
                starts.insert(c.lparen.0 as u32, c.fun.pos().0 as u32);
            }
            match n {
