...
/// Stub dependency sources under `<fixture dir>/stub/<import path>/*.go`.
///
/// Without an importer the checker cannot resolve `import "fmt"` at all, so no
/// `PkgName` object exists and nothing lands in `Info.Uses` — a fixture that
/// looks like it exercises a use-site rewrite silently exercises nothing. Same
/// mechanism the `guff-style` harness uses.
...
pub fn typecheck_fixture(name: &str, pkg_id: &str, file: &str) -> Arc<Package> {
    ...
    let deps: Vec<(&str, &Path)> = stubs
        ...
        .collect();
    typecheck_with_deps(pkg_id, &dir.join(file), &deps)
}
...
///
/// A suggested fix's replacement text and span appear in no compat key, so a
/// linter can report perfectly and rewrite the file wrongly — or rewrite half
/// of what it should. Asserting on the edits here is the unit-level net under
/// `compat/fix/`.
